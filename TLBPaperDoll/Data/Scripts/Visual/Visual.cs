    ...
    public class Visual : MySessionComponentBase
    {
        ...
        private void ExecuteVSearchUpdate(MyEntity controlEnt)
        {
            HandEx(() =>
            {
                ...
                var ent = (MyAPIGateway.Session.Player.Controller.ControlledEntity as IMyCubeBlock).CubeGrid;
        ...
        private void ManEntFoc(MyEntity cEnt)
        {
            ...
            var ent = MyAPIGateway.Session?.Player?.Character?.Components?.Get<MyTargetLockingComponent>()?.TargetEntity;
            if (ent == null)
            {
                ClearAVis();
            }
